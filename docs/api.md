...
foreach ($posts as $post) {
    ...
    echo $post->created_at; // 如果配置了 casts，会自动格式化为字符串
}
```
...
// 3. 执行验证
$validator = (new Validate())->rules($rules);
...
$validator = (new Validate())->rules($rules)->messages($messages);
$validator->validate($data);
```
...
// ===== GROUP BY + HAVING =====
// 聚合查询需用 Connection::raw()，select() 不支持含空格/括号的表达式
$stats = $connection->raw('SELECT user_id, COUNT(*) as total FROM orders GROUP BY user_id HAVING total > ?', [5])->fetchAll();
...
$c->each(function($item, $key) { echo $item; });
...
// 通过 Application 获取容器中的服务实例
$db = \core\Application::getInstance()->getContainer()->get(\db\Connection::class);
```
...
| 方法 | 签名 | 说明 |
|------|------|------|
| `force` | `force(): static` | 启用强制删除模式（物理删除），返回新实例 |
| `trashed` | `trashed(): bool` | 检查当前实例是否已被软删除 |
| `restore` | `restore(): bool` | 恢复一个被软删除的模型 |
| `withTrashed` | `static withTrashed(): static` | 包含软删除记录的查询 |
| `onlyTrashed` | `static onlyTrashed(): static` | 仅查询软删除的模型 |
...
$post = Post::find(1);
$post->delete($post->id);       // 软删除（默认行为）
$all = Post::withTrashed()->fetchAll();  // 包含软删除记录
$post->restore();               // 恢复
// 强制物理删除（force() 返回启用强制模式的实例，再调用 delete）
$post = Post::find(1);
$post->force()->delete($post->id);
```
